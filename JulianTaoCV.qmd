...
Since 2009 I have authored `r npub` research papers and filed `r npatent` patents.
...
jttalks |>
  ...
  mutate(
    ...
    Talk = paste("\\textit{", Title, "}. ", Meeting, ". ", Location, sep = ""),
  ) |>
  select(Time, Talk) |>
...
jtorgconfs |>
  ...
  baretable(align = "lp{0.7in}p{4.2in}p{2.2in}")
```
...
jteditorial |>
  ...
  baretable(align = "lp{1in}p{1.8in}p{4in}")
```
...
allstudents |>
  ...
  baretable(align = "lp{0.8in}p{2in}p{0.8in}p{3.1in}")
```
...
allstudents |>
  filter(Degree == "HSRA") |>
  select(Years, Name, Institute, TopicNotes) |>
  baretable(align = "lp{0.8in}p{1.8in}p{2.2in}p{2.1in}")
```
...
allgrants |>
  ...
  baretable(align = "lp{1in}p{4.7in}p{1.3in}")
```
